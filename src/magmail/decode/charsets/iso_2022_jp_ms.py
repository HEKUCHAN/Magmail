...
from magmail.decode.mappings import jis90_map, jis78_map, jis78_target, nec_ibm_map, nec_map

def decode(byte, error="strict"):
    byte_array = iter(array.array("B", byte))
    byte_decoded = []
    decode_type = {
        "ascii": False,
        "katakana": False,
        ...
        while True:
            ...
            if byte_1 == 0x1B:
                ...
                if (byte_1, byte_2) == (0x28, 0x42):
                    ...
                    raise UnicodeDecodeError(f"Unknown escape suquence \'{byte_1}{byte_2}\'")
                ...
                if is_decode_type('ascii') and 0x00 <= byte_1 and byte_1 <= 0x7f:
                    ...
                    if (
                        (
                         (0x21 <= byte_1 and byte_1 <= 0x28) or
                         (0x30 <= byte_1 and byte_1 <= 0x74)
                        ) and
                        ...
                        if joined_byte in jis78_target:
                            byte_decoded.append(
                                jis78_map.get(joined_byte, 0x0000)
                            )
                        else:
                            byte_decoded.append(
                                jis90_map.get(joined_byte, 0x0000)
                            )
                    ...
                    if (
                        ...
                        joined_byte = join_hex(byte_1, byte_2)
                        byte_decoded.append(
                            chr(jis90_map.get(joined_byte, 0x0000))
                        )
                    elif (
                        (byte_1 == 0x2D) and
                        (0x21 <= byte_2 and byte_2 <= 0x7E)
                        ...
                        (0x79 <= byte_1 and byte_1 <= 0x7C) and
                        (0x21 <= byte_2 and byte_2 <= 0x7E)
                    ):
                        # NEC選定IBM拡張文字 (`ESC $ B`)
                        joined_byte = join_hex(byte_1, byte_2)
